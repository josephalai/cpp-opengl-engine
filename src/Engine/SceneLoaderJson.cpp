    ...
    std::vector<PhysicsGroundCfg>& physicsGroundCfgs)
{
    ...
    // -----------------------------------------------------------------------
    // Random entities — REMOVED (GEA Step 5.1)
    //
    // Random entity placement is now handled by the offline Asset Baker.
    // The ChunkManager loads pre-baked binary .dat files and fires
    // EntitySpawnCallback to instantiate entities at the exact positions
    // computed during the bake step.  This eliminates PRNG synchronisation
    // issues and provides instantaneous loading.
    // -----------------------------------------------------------------------
